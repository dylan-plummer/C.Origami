...
def single_prediction(output_path, celltype, chr_name, start, model_path, seq_path, ctcf_path, atac_path, image_scale=256):
        ...
        plot = plot_utils.MatrixPlot(output_path, pred, 'prediction', celltype, 
                                        chr_name, start)
        plot.plot(vmin=1, vmax=5)

        hic = data_feature.HiCFeature(path = f'data/corigami_data/data/hg19/{celltype}/hic_matrix/{chr_name}.npz')
        mat = hic.get(start)
